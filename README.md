...
Fouris has run on a simulator for over 99 hours (and would have continued for a longer period except my iMac had a force restart).
...
In most game modes, once a user's piece freezes (eg, no longer able to move) in place above the edge of the bucket, the game is over.
